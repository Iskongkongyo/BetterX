...
const scriptPath = path.join(__dirname, '..', '更好的X（BetterX）v3.2.3.js');
...
for (const action of ['reply', 'repost', 'like', 'bookmark', 'share', 'download']) {
...
for (const operation of ['FavoriteTweet', 'UnfavoriteTweet', 'CreateRetweet', 'DeleteRetweet', 'CreateBookmark', 'DeleteBookmark']) {
  assert.match(source, new RegExp(`${operation}: ['"][A-Za-z0-9_-]{8,100}['"]`), `缺少 ${operation} 操作标识`);
}
assert.match(source, /cloneNode\(true\)/, '应动态复用 X 原生操作栏 DOM 与样式');
...
source = source.replace(startupPattern, `
  globalThis.__betterxNotificationTest = {
    ...
    learnNativePostActionQueryId,
    nativePostActionQueryIds,
  ...
  globalObjects: {
    tweets: {
      ...
      90: {
        ...
        favorite_count: 12, retweet_count: 3, reply_count: 2, favorited: true, retweeted: true, bookmarked: true,
        ext_views: { count: '599' },
        entities: {
...
assert.equal(parsed.posts[0].favorited, true);
assert.equal(parsed.posts[0].reposted, true);
assert.equal(parsed.posts[0].bookmarked, true);
...
api.learnNativePostActionQueryId('https://x.com/i/api/graphql/NEW_QUERY_ID/FavoriteTweet');
assert.equal(api.nativePostActionQueryIds.get('FavoriteTweet'), 'NEW_QUERY_ID');

api.state.notificationTimelineActive = true;
assert.equal(api.getCurrentSourceInfo().type, 'notification_subscriptions');

console.log('BetterX v3.2.3 native action tests passed');
